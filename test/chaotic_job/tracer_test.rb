...
class ChaoticJob::TracerTest < ActiveJob::TestCase
  ...
  test "capture traces line events" do
    ...
    assert_equal 3, result.count { |it| it.type == :line }
  end
  ...
  test "capture traces call and return events" do
    ...
    assert_equal 1, result.count { |it| it.type == :call }
    assert_equal 1, result.count { |it| it.type == :return }
    assert_equal 2, result.count { |it| it.type == :line }
  end
  ...
  test "constraint filters events" do
    ...
    assert_equal 0, result.count { |it| it.type == :call }
    assert_equal 0, result.count { |it| it.type == :return }
    assert_equal 2, result.count { |it| it.type == :line }
  end
  ...
  test "line key format and stuff" do
    ...
    assert_equal 0, result.count { |it| it.type == :call }
    assert_equal 0, result.count { |it| it.type == :return }
    assert_equal 1, result.count { |it| it.type == :line }
    assert result.all? { |it| it.key.include?(__FILE__) && it.key.include?(":") }
  end
  ...
  test "call key format for instance methods" do
    ...
    assert_equal 1, result.count { |it| it.type == :return }
    assert_equal 2, result.count { |it| it.type == :line }
    assert_equal 1, result.count { |it| it.type == :call && it.key.include?("#test_instance_method") }
  end
  ...
  test "excludes tracer class from tracing" do
    ...
    assert_equal 0, result.count { |it| it.key.to_s.include?("ChaoticJob::Tracer") }
  end
